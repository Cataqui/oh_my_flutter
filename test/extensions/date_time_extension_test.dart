...
void main() {
  group('DateTimeExtension', () {
    ...
    group('when all callbacks are supplied and the matched bucket fires directly', () {
      ...
      test(
        'when 500 milliseconds have elapsed, it should invoke onMillisecondsAgo with 500',
        () {
          withClock(Clock.fixed(fixedNow), () {
      ...
      test(
        'when 999 milliseconds have elapsed, it should invoke onMillisecondsAgo with 999',
        () {
          withClock(Clock.fixed(fixedNow), () {
      ...
      test(
        'when exactly 1 second has elapsed, it should invoke onSecondsAgo with 1',
        () {
          withClock(Clock.fixed(fixedNow), () {
      ...
      test(
        'when 59 seconds have elapsed, it should invoke onSecondsAgo with 59',
        () {
          withClock(Clock.fixed(fixedNow), () {
      ...
      test(
        'when exactly 60 seconds have elapsed, it should invoke onMinutesAgo with 1',
        () {
          withClock(Clock.fixed(fixedNow), () {
      ...
      test(
        'when 59 minutes have elapsed, it should invoke onMinutesAgo with 59',
        () {
          withClock(Clock.fixed(fixedNow), () {
      ...
      test(
        'when exactly 60 minutes have elapsed, it should invoke onHoursAgo with 1',
        () {
          withClock(Clock.fixed(fixedNow), () {
      ...
      test(
        'when 23 hours have elapsed, it should invoke onHoursAgo with 23',
        () {
          withClock(Clock.fixed(fixedNow), () {
      ...
      test(
        'when exactly 24 hours have elapsed, it should invoke onDaysAgo with 1',
        () {
          withClock(Clock.fixed(fixedNow), () {
      ...
      test(
        'when 30 days have elapsed, it should invoke onMonthsAgo with the correct month count',
        () {
          withClock(Clock.fixed(fixedNow), () {
      ...
      test(
        'when 11 months have elapsed, it should invoke onMonthsAgo with 11',
        () {
          withClock(Clock.fixed(fixedNow), () {
      ...
      test(
        'when 12 months have elapsed, it should invoke onYearsAgo with 1',
        () {
          withClock(Clock.fixed(fixedNow), () {
      ...
      test(
        'when the day-of-month is not yet reached, it should not count a full month',
        () {
          withClock(Clock.fixed(fixedNow), () {
      ...
      test(
        'when several years have elapsed, it should invoke onYearsAgo with the correct year count',
        () {
          withClock(Clock.fixed(fixedNow), () {
    ...
    group('when fallback is finer and the matched bucket callback is missing', () {
      test(
        'when a finer count-bucket is supplied, it should invoke it with the recomputed count',
        () {
          withClock(Clock.fixed(fixedNow), () {
            // matched = hours (3), finer = minutes (4)
            final past = fixedNow.subtract(
      ...
      test(
        'when only the finest count-bucket is supplied, it should invoke that bucket',
        () {
          withClock(Clock.fixed(fixedNow), () {
      ...
      test(
        'when only onNow is supplied (collapse-to-now edge), it should invoke onNow',
        () {
          withClock(Clock.fixed(fixedNow), () {
      ...
      test(
        'when no finer callback (including onNow) is supplied and onMiss is supplied, it should invoke onMiss',
        () {
          withClock(Clock.fixed(fixedNow), () {
    ...
    group('when fallback is coarser and the matched bucket callback is missing', () {
      test(
        'when a coarser count-bucket is supplied, it should invoke it with the recomputed count',
        () {
          withClock(Clock.fixed(fixedNow), () {
      ...
      test(
        'when only onYearsAgo is supplied, it should invoke onYearsAgo (nearest-coarser precedence)',
        () {
          withClock(Clock.fixed(fixedNow), () {
      ...
      test(
        'when only onNow is supplied (no coarser), it should invoke onMiss',
        () {
          withClock(Clock.fixed(fixedNow), () {
      ...
      test(
        'when no coarser callback is supplied and onMiss is null, it should throw ArgumentError',
        () {
          withClock(Clock.fixed(fixedNow), () {
            final past = fixedNow.subtract(const Duration(hours: 5));
            expect(
    ...
    // ─────────────────────────────────────────────────────────────────────────
    group(
      'when fallback is bidirectional and the matched bucket callback is missing',
      () {
        test(
          'when both finer and coarser count-buckets are supplied, it should prefer finer',
          () {
            withClock(Clock.fixed(fixedNow), () {
        ...
        test(
          'when only a coarser count-bucket is supplied, it should invoke that coarser bucket',
          () {
            withClock(Clock.fixed(fixedNow), () {
        ...
        test(
          'when onNow is last resort after no other bucket, it should invoke onNow',
          () {
            withClock(Clock.fixed(fixedNow), () {
        ...
        test(
          'when onNow is available alongside a coarser count-bucket, it should prefer the coarser over onNow',
          () {
            withClock(Clock.fixed(fixedNow), () {
        ...
        test(
          'when no callback is supplied anywhere and onMiss is supplied, it should invoke onMiss',
          () {
            withClock(Clock.fixed(fixedNow), () {
              final past = fixedNow.subtract(const Duration(hours: 5));
              final result = past.timeAgo(
        ...
        test(
          'when no callback is supplied anywhere and onMiss is null, it should throw ArgumentError',
          () {
            withClock(Clock.fixed(fixedNow), () {
              final past = fixedNow.subtract(const Duration(hours: 5));
              expect(
