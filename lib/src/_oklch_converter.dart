...
final class _OklchConverter {
  ...
  static Color toColor(double lightness, double chroma, double hue, {ColorSpace colorSpace = ColorSpace.sRGB}) {
    _validateComponents(lightness: lightness, chroma: chroma, hue: hue);

    final normalizedLightness = lightness.clamp(0, 1).toDouble();
    if (normalizedLightness == 0) return _colorFromChannels(0, 0, 0, colorSpace);
    if (normalizedLightness == 1) return _colorFromChannels(1, 1, 1, colorSpace);

    final normalizedChroma = math.max(0, chroma).toDouble();
    final normalizedHue = ((hue % 360) + 360) % 360;
    final linearRgb = _toLinearRgb(normalizedLightness, normalizedChroma, normalizedHue, colorSpace);
    if (colorSpace == ColorSpace.extendedSRGB || _isInGamut(linearRgb)) {
      return _linearRgbToColor(linearRgb, colorSpace);
    }

    return _linearRgbToColor(_mapToGamut(normalizedLightness, normalizedChroma, normalizedHue, colorSpace), colorSpace);
  }

  static void _validateComponents({required double lightness, required double chroma, required double hue}) {
  ...
  static (double, double, double) _linearRgbToOklab(double red, double green, double blue, ColorSpace colorSpace) {
    switch (colorSpace) {
  ...
  static (double, double, double) _linearSrgbToOklab(double red, double green, double blue) {
  ...
  static (double, double, double) _xyzToOklab(double x, double y, double z) {
    final lRoot = _cbrt(0.819022437996703 * x + 0.3619062600528904 * y - 0.1288737815209879 * z);
    final mRoot = _cbrt(0.0329836539323885 * x + 0.9292868615863434 * y + 0.0361446663506424 * z);
    final sRoot = _cbrt(0.0481771893596242 * x + 0.2642395317527308 * y + 0.6335478284694309 * z);
  ...
  static (double, double, double) _toLinearRgb(double lightness, double chroma, double hue, ColorSpace colorSpace) {
  ...
  static (double, double, double) _mapToGamut(double lightness, double chroma, double hue, ColorSpace colorSpace) {
    var minimumChroma = 0.0;
    var maximumChroma = chroma;
    var minimumIsInGamut = true;
    var clipped = _clipLinearRgb(_toLinearRgb(lightness, chroma, hue, colorSpace));
    ...
    while (maximumChroma - minimumChroma > _gamutEpsilon) {
      ...
      if (difference < _justNoticeableDifference) {
        if (_justNoticeableDifference - difference < _gamutEpsilon) return clipped;
  ...
  static double _deltaEok(double lightness, double chroma, double hue, (double, double, double) clippedOklab) {
    final hueRadians = hue * math.pi / 180;
    final a = chroma * math.cos(hueRadians);
    final b = chroma * math.sin(hueRadians);
    ...
    return math.sqrt(lightnessDifference * lightnessDifference + aDifference * aDifference + bDifference * bDifference);
  }

  static (double, double, double) _clipLinearRgb((double, double, double) linearRgb) {
    return (linearRgb.$1.clamp(0.0, 1.0), linearRgb.$2.clamp(0.0, 1.0), linearRgb.$3.clamp(0.0, 1.0));
  }

  static Color _linearRgbToColor((double, double, double) linearRgb, ColorSpace colorSpace) {
  ...
  static Color _colorFromChannels(double red, double green, double blue, ColorSpace colorSpace) {
    return Color.from(alpha: 1, red: red, green: green, blue: blue, colorSpace: colorSpace);
  }
}
